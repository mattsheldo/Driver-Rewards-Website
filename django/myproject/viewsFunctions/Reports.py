...
class ReportItem:
    def __init__(self, name, cost, datePurchased, compName):
        self.name = name
        self.cost = cost
...
# monthInt is the month number (1-12)
# V IMPORTANT: monthInt does NOT start at 0, meaning January = 1
def getSalesOverTime(monthInt, year):
    report = []
    ...
    try:
        ...
        # Acquire each item purchased from the given company during the given month
        myCursor = mydb.cursor()
        query = "SELECT Product_Name, Point_Total, PointsPerDollar, Date_, Name_ FROM Purchase_History JOIN Employers as E ON Employer_ID = E.ID WHERE Date_ > '" + monthBegin + "' AND Date_ < '" + monthEnd + "' ORDER BY Date_;"
        try:
            # Execute query and get results
            myCursor.execute(query)
            myResults = myCursor.fetchall()
            ...
            for item in myResults:
                ...
                itemDate = str(item[3])[0:10].replace("-", "/")
                compName = item[4]
                report.append(ReportItem(itemName, itemCost, itemDate, compName))
        except Exception as e:
            print("getSalesOverTime(): Failed to query database: " + str(e))
        finally:
            myCursor.close()
    except Exception as e:
        print("getSalesOverTime(): Failed to connect: " + str(e))
    finally:
        mydb.close()
        return report
